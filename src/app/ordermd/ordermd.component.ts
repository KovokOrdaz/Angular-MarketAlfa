...
import { DialogordermdComponent } from './dialogordermd/dialogordermd.component';
...
@Component({
  selector: 'app-ordermd',
  templateUrl: './ordermd.component.html',
  styleUrls: ['./ordermd.component.scss']
})

export class OrdermdComponent implements OnInit {
  ...
  openCreate() {
    const dialogRef = this.dialog.open(DialogordermdComponent, {
      maxWidth: '100vw',
      maxHeight: '100vh',
      height: '100%',
      width: '100%',
  ...
  openEdit(entity: OrderX) {
    const dialogRef = this.dialog.open(DialogordermdComponent, { data: entity });
    dialogRef.afterClosed().subscribe(finish => { this.read() });
    this.clearSelection();
  }
