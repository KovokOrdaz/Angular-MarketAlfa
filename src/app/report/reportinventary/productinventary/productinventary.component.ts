...
@Component({
  selector: 'app-productinventary',
  templateUrl: './productinventary.component.html',
  styleUrls: ['./productinventary.component.scss']
})
export class ProductinventaryComponent implements OnInit {
  single: any[] = [{
    "name": "Germany",
    "value": 8940000
  },
  ...
  constructor(private api: ApiinventoryService) {
    this.api.read().subscribe(x => {
      ...
      for (let i = 0; i < this.rows.length; i++) {
        this.single.push({ name: this.rows[i].name + " - " + this.rows[i].measure, value: this.rows[i].amountX });
      }
  ...
  createPDF() {
    ...
    html2canvas(DATA, options).then((canvas) => {
      ...
      docResult.save("Grafica_Inventario_MarketAlfa.pdf");
    });
  }
