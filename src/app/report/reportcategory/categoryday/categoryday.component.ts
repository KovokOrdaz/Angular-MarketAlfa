...
@Component({
  selector: 'app-categoryday',
  templateUrl: './categoryday.component.html',
  styleUrls: ['./categoryday.component.scss']
})
export class CategorydayComponent implements OnInit {
  ...
  constructor(private api: ApibuyService, public apiBusiness: ApiBusinessService) {
    ...
    this.api.reportCategoryDay().subscribe(x => {
      this.rows = x.data;
      this.temp = this.rows;
      this.single = [];
