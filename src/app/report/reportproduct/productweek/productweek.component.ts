...
@Component({
  selector: 'app-productweek',
  templateUrl: './productweek.component.html',
  styleUrls: ['./productweek.component.scss']
})
export class ProductweekComponent implements OnInit {
  ...
  constructor(private api: ApibuyService, public apiBusiness: ApiBusinessService) {
    ...
    this.api.reportProductWeek().subscribe(x => {
      this.rows = x.data;
      this.temp = this.rows;
      this.single = [];
