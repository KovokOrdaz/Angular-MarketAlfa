...
@Component({
  selector: 'app-measuremount',
  templateUrl: './measuremount.component.html',
  styleUrls: ['./measuremount.component.scss']
})
export class MeasuremountComponent implements OnInit {
  ...
  constructor(private api: ApibuyService, public apiBusiness: ApiBusinessService) {
    this.api.reportMeasureMounth().subscribe(x => {
      this.rows = x.data;
      this.temp = this.rows;
      this.single = [];
