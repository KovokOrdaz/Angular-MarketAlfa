...
@Component({
  selector: 'app-measureweek',
  templateUrl: './measureweek.component.html',
  styleUrls: ['./measureweek.component.scss']
})
export class MeasureweekComponent implements OnInit {
  ...
  constructor(private api: ApibuyService, public apiBusiness: ApiBusinessService) {
    this.api.reportMeasureWeek().subscribe(x => {
      this.rows = x.data;
      this.temp = this.rows;
      this.single = [];
