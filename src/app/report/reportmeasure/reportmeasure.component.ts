...
@Component({
  selector: 'app-reportmeasure',
  templateUrl: './reportmeasure.component.html',
  styleUrls: ['./reportmeasure.component.scss']
})
export class ReportmeasureComponent implements OnInit {
  ...
  constructor(private api: ApibuyService, public apiBusiness: ApiBusinessService) {
    this.api.reportMeasure().subscribe(x => {
      this.rows = x.data;
      this.temp = this.rows;
      this.single = [];
